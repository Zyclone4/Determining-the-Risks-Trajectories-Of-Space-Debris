...
def create_app():
    ...
    from data_pipeline import load_cache, build_and_save_dataset
    ...
    async def risks(limit: int = 10000, minRisk: float = 0.0):
        ...
        for _, row in obj.iterrows():
            ...
            scored.append({
                ...
                "minutesToApproach": (int(row["nearest_approach_step"]) * 5) if not pd.isna(row.get("nearest_approach_step")) and int(row.get("nearest_approach_step", -1)) >= 0 else None,
                "closestApproachObject": name_map.get(int(row["nearest_approach_partner"]), {}).get("name", "Unknown") if not pd.isna(row.get("nearest_approach_partner")) else None,
                "closestApproachObjectId": int(row["nearest_approach_partner"]) if not pd.isna(row.get("nearest_approach_partner")) else None,
    ...
    async def analyze(req: AnalyzeRequest):
        try:
            custom_t0 = datetime.fromisoformat(req.startTime.replace("Z", "+00:00"))
        except ValueError:
            return {"error": f"Invalid startTime format: {req.startTime}"}

        records = load_cache()
        if records is None:
            return {"error": "No cached TLE data available. Run data_pipeline.py first."}

        live_path = CACHE_DIR / "propagated_features_live.parquet"
        df = build_and_save_dataset(records, t0=custom_t0, output_path=live_path)
        if df is None:
            return {"error": "Propagation failed for the given start time."}
        ...
        all_scores = {}
        with torch.no_grad():
            for nid, group in df.groupby("NORAD_CAT_ID"):
                if group["debris_status"].iloc[0] != 1:
                    continue
                group_sorted = group.sort_values("step")
                feats = np.nan_to_num(
                    group_sorted[GRU_FEATURES].values, nan=0.0
                ).astype(np.float32)
                x = torch.from_numpy(feats).unsqueeze(0)
                pred = model(x).squeeze(0).numpy()
                all_scores[str(nid)] = float(np.clip(np.max(pred), 0.0, 1.0))
        ...
        for _, row in obj.iterrows():
            ...
            risk = round(all_scores.get(str(norad), 0.0), 4)
            if risk >= 0.70:
                label = "Critical"
            elif risk >= 0.45:
                label = "Watch"
